---
title: 在 macOS 上查看并解析 electron crash
date: 2024-10-15T20:56:00+08:00
tags:
  - electron
---
...
在 [chromium crash-reports] 中，我们可以看到 crash 文件被保留在 `~/Library/Application\ Support/Chromium/Crashpad/completed` 目录下。但是由于我们是 electron 应用，没有“提交”这个过程，所以 crash 文件会被保留在 `~/Library/Application\ Support/Chromium/Crashpad/pending` 目录下。

在查找前，记得把上面的目录中的 `Chromium` 替换为你的应用名，比如 `OOMOL Studio`。

如果你 crash 的次数比较多，那么你会在这个目录下看到很多的 _*.dmp_ 文件，一般而言，我们需要分析的都是最近一次 crash 的，所以最新的文件就是我们需要的。

## 解析 crash 文件

### 使用 `breakpad`

`breakpad` 是一个 Google 开源的 crash 分析工具，也是专门为 chromium 开发的。我们可以使用这个工具去解析我们的 _dmp_ 文件。
